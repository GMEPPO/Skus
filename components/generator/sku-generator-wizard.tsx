...
export function SkuGeneratorWizard({
  families,
}: {
  ...
  if (!family) {
    return <div className="text-sm text-slate-400">Sem famílias configuradas.</div>;
  }
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          <Card className="p-4">
            <label className="space-y-2">
              <span className="text-sm font-medium text-slate-200">Família</span>
              <select
                value={family.id}
        ...
        <div className="space-y-4">
          <Card className="space-y-4 p-4">
            ...
            <div className="rounded-xl border border-slate-700 bg-slate-950/50 p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Progressão</p>
      ...
      <div className="sticky bottom-4 z-20 rounded-2xl border border-amber-500/30 bg-slate-950/95 p-4 shadow-2xl shadow-black/30 backdrop-blur">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.24em] text-amber-300">Designação</p>
            <p className="mt-2 text-base text-slate-100">
              {designation || "Seleciona os campos para construir a designação final."}
            </p>
          </div>
          <Button disabled={completedCount !== family.levels.length}>Gerar SKU</Button>
        </div>
      </div>
    </div>
  );
