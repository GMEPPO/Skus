...
-- ===========================================================================
-- R5 GOLDEN HARNESS — NOT A MIGRATION. Ends in ROLLBACK.
-- Replaces migration COMMIT with ROLLBACK (do not wrap original COMMIT file).
-- ===========================================================================
...
$j_G1_two_words_and_empty$::jsonb;
  ...
  raise notice '% typescript=% postgres=% match=%', 'G1_two_words_and_empty', v_expected, v_hash, (v_hash = v_expected);
...
$j_G2_empty_participant$::jsonb;
  v_hash := skus_private.compute_selection_fingerprint(v_snap);
  raise notice '% typescript=% postgres=% match=%', 'G2_empty_participant', v_expected, v_hash, (v_hash = v_expected);
  if v_hash is distinct from v_expected then
    v_fail := v_fail + 1;
  end if;
...
$j_G3_shuffled_order$::jsonb;
  v_hash := skus_private.compute_selection_fingerprint(v_snap);
  raise notice '% typescript=% postgres=% match=%', 'G3_shuffled_order', v_expected, v_hash, (v_hash = v_expected);
  if v_hash is distinct from v_expected then
    v_fail := v_fail + 1;
  end if;
...
$j_G4_omit_non_participating_empty$::jsonb;
  v_hash := skus_private.compute_selection_fingerprint(v_snap);
  raise notice '% typescript=% postgres=% match=%', 'G4_omit_non_participating_empty', v_expected, v_hash, (v_hash = v_expected);
  if v_hash is distinct from v_expected then
    v_fail := v_fail + 1;
  end if;
...
$j_G5_non_participating_word_null$::jsonb;
  v_hash := skus_private.compute_selection_fingerprint(v_snap);
  raise notice '% typescript=% postgres=% match=%', 'G5_non_participating_word_null', v_expected, v_hash, (v_hash = v_expected);
  if v_hash is distinct from v_expected then
    v_fail := v_fail + 1;
  end if;
...
$j_G6_label_change_same_hash$::jsonb;
  v_hash := skus_private.compute_selection_fingerprint(v_snap);
  raise notice '% typescript=% postgres=% match=%', 'G6_label_change_same_hash', v_expected, v_hash, (v_hash = v_expected);
  if v_hash is distinct from v_expected then
    v_fail := v_fail + 1;
  end if;
...
$j_G7_segment_change_diff_hash$::jsonb;
  v_hash := skus_private.compute_selection_fingerprint(v_snap);
  raise notice '% typescript=% postgres=% match=%', 'G7_segment_change_diff_hash', v_expected, v_hash, (v_hash = v_expected);
  if v_hash is distinct from v_expected then
    v_fail := v_fail + 1;
  end if;
...
{"version":2,"codeFormatVersion":1,"category":{"id":"11111111-1111-1111-1111-111111111111","slug":"cosmetica","name":"Cosmetica"},"levels":[{"levelId":"22222222-2222-2222-2222-222222222222","key":"k1","label":"L1","sortOrder":1,"isEnabled":true,"isRequired":false,"participatesInCode":true,"codeSegment":"ALG","selection":{"kind":"word","wordId":"33333333-3333-3333-3333-333333333333","label":"X","referenceCode":"ALG","includeInDesignation":true,"designations":{"pt":"A","es":"A","en":"A"}}},{"levelId":"44444444-4444-4444-4444-444444444444","key":"e2","label":"E2","sortOrder":2,"isEnabled":true,"isRequired":false,"participatesInCode":true,"codeSegment":"000","selection":{"kind":"empty"}}]}
$j_G8_uuid_uppercase_normalized$::jsonb;
  v_hash := skus_private.compute_selection_fingerprint(v_snap);
  raise notice '% typescript=% postgres=% match=%', 'G8_uuid_uppercase_normalized', v_expected, v_hash, (v_hash = v_expected);
  if v_hash is distinct from v_expected then
    v_fail := v_fail + 1;
  end if;

  -- measures wrong types must fail parse path via helper contract (type guard is in builder;
  -- here we assert NaN/Inf helpers)
  begin
    perform skus_private.parse_positive_measure('NaN');
    raise exception 'NaN should fail';
  exception when others then
    if sqlerrm is distinct from 'invalid_payload' then raise; end if;
  end;
  begin
    perform skus_private.parse_positive_measure('inf');
    raise exception 'inf should fail';
  exception when others then
    if sqlerrm is distinct from 'invalid_payload' then raise; end if;
  end;

  if v_fail > 0 then
    raise exception 'R5 golden mismatch count=%', v_fail;
  end if;
  raise notice 'R5 golden harness OK: all vectors matched TS hashes';
end;
$golden$;
